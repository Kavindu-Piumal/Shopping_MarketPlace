  ...
  const handleSubmitSubCategory = async (e) => {
    try {
      ...
      const response = await Axios({
        url: summaryApi.updateSubCategory.url,
  ...
  if (!isOwner) {
    return (
      <section className="fixed top-0 right-0 bottom-0 left-0 bg-black bg opacity-80 flex items-center justify-center z-50">
        <div className="w-full max-w-md bg-white p-6 rounded-lg shadow-2xl border border-green-300 relative animate-fadeIn text-center">
          <h2 className="font-semibold text-red-600">You don't have permission to edit this subcategory.</h2>
          <button onClick={close} className="mt-4 px-4 py-2 bg-green-500 text-white rounded">Close</button>
        </div>
      </section>
    );
  }
